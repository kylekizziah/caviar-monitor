...
PREFERRED_SIZES_G = [28,30,50,100,113,125,250]
...
def safe_get(sess, url, timeout=20):
    try:
        ...
        if VERBOSE_LOG: print("GET exception:", e)
        return None
...
def vendor_state(vendor): return VENDOR_HOME_STATE.get(vendor,"US")
...
def scrape_product(sess,url,vendor):
    ...
    if mentions_non_sturgeon(text): return []
    title = (soup.title.string if soup.title else "") or ""
    h1 = soup.find("h1")
    name = (h1.get_text(" ",strip=True) if h1 else title).strip()
    if is_accessory_name_only(name): return []
    if not CAVIAR_WORD.search(text.lower()) and "caviar" not in name.lower(): return []
    sp,lat = extract_species(name+" "+text)
    if not sp: return []
    price_match = MONEY_RE.search(text)
    ...
    return [{
        ...
        "species":sp,"species_latin":lat,"grade":None,
        "currency":"USD","price":price,"size_g":size_g,
        "size_label":sl,"per_g":per_g,"origin_state":vendor_state(vendor)
    }]
...
def crawl_site(cfg, deadline):
    ...
    vendor=cfg.get("name")
    sess=make_session()
    urls=set(cfg.get("seed_product_urls") or [])
    for u in urls:
        if datetime.utcnow()>deadline: break
        results.extend(scrape_product(sess,u,vendor))
        time.sleep(0.1)
    return results
