...
def scrape_product(url, vendor, default_species=None):
    ...
    # Build a comprehensive text surface (improves species detection)
    texts = []
    ...
    # meta titles/descriptions often carry species
    ...
    if not (CAVIAR_WORD.search((name + " " + page_text).lower())): return []
    if is_accessory(name) or is_accessory(page_text): return []
    if is_non_sturgeon(name) or is_non_sturgeon(page_text): return []

    # species (required; but allow YAML default_species fallback if provided)
    species, latin = extract_species(name)
    if not species:
        species, latin = extract_species(page_text)
    if not species and default_species:
        # allow only known sturgeon defaults
        s2, l2 = extract_species(default_species)
        if s2:
            species, latin = s2, l2

    if not species:
        return []  # still require species to be known

    gr_rank, grade_label = grade_rank(name + " " + page_text)
    out=[]
...
def crawl_site(site_cfg, deadline):
    ...
    default_species = site_cfg.get("default_species")  # NEW: YAML-provided species fallback
    ...
    for seed in seed_urls[:MAX_LINKS_PER_SITE]:
        ...
        if looks_like_product_url(low) and domain_ok(seed):
            results += scrape_product(seed, vendor, default_species=default_species)
            time.sleep(0.1)
    ...
    for start in start_urls:
        ...
        for u in list(links)[:MAX_LINKS_PER_SITE]:
            if datetime.utcnow() > deadline:
                break
            results += scrape_product(u, vendor, default_species=default_species)
            time.sleep(0.1)
